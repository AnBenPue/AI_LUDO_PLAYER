...
void game::reset()
{
    ...
    { //without & we're changing the copy made in auto rather than the player_position
        i = -1;
    }
    color = 3;
}
...
// modified version copied from alberto's:
int game::isOccupied(int index)
{ //returns number of people of another color
    ...
    if (index != 99)
    {
        ...
        {
            if (i < static_cast<size_t>(color) * 4 || i >= static_cast<size_t>(color) * 4 + 4)
            { //Disregard own players
...
int game::isGlobeCustom(int index)
{
    if (index % 13 == 0)
        return 1;
    else if ((index - 8) % 13 == 0)
        return 2;
    else if (isOccupied(index) > 1)
    {
        return 3;
    }
    return 0;
...
void game::movePiece(int relative_piece)
{
    ...
    else
    {
        ...
        if (DEBUG)
            std::cout << "color: " << color << " pos: " << relative_pos << " + " << dice_result << " = " << relative_pos + dice_result;
        //add dice roll
...
{
    static int games_played = 0;           //ADDED
    static std::vector<int> players(4, 0); //ADDED
    game_complete = win;
    turn_complete = true;
    if (game_complete)
    {
        ...
        cout << "  --------------------------------" << endl;
        cout << "  | Games played:   " << setw(10) << games_played << "   |" << endl;
        cout << "  --------------------------------" << endl;
        cout << "  | Player 0: " << setw(8) << players[0] << setw(8) << int(players[0] / float(games_played) * 100) << "%  |" << endl;
        cout << "  | Player 1: " << setw(8) << players[1] << setw(8) << int(players[1] / float(games_played) * 100) << "%  |" << endl;
        cout << "  | Player 2: " << setw(8) << players[2] << setw(8) << int(players[2] / float(games_played) * 100) << "%  |" << endl;
        cout << "  | Player 3: " << setw(8) << players[3] << setw(8) << int(players[3] / float(games_played) * 100) << "%  |" << endl;
        cout << "  --------------------------------" << endl;
        save_data("Data/DATA_GAME_WINERS.txt",games_played,color,(players[0] / float(games_played) * 100),(players[1] / float(games_played) * 100),(players[2] / float(games_played) * 100),(players[3] / float(games_played) * 100));
...
{
    if (DEBUG)
        std::cout << "color:     relative pos => fixed\n";
    while (!game_complete)
    {
        if (turn_complete)
        {
...
void game::save_data(string filepath, double d1,double d2,double d3,double d4,double d5,double d6)
{
    std::ofstream file;
